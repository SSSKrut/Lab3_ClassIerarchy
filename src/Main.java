...
public class Main {
    public static void main(String[] args) {
        Place DarkStuffyRoom=new Place("Комната","Их комната, тёмная душная комната.");
        ...
        Vincent vincent = new Vincent("Винсент");
        Sister sister = new Sister("сестра");
        Mother mother = new Mother("мать");

        DarkStuffyRoom.addActor(vincent);
        DarkStuffyRoom.addActor(sister);
        DarkStuffyRoom.addActor(mother);
        ...
        mother.items.add(ladle); mother.items.add(ticket);
        vincent.items.add(plateV);
        plateS.items.add(foodnaggets);
        sister.items.add(plateS);

        vincent.setMemory(DarkStuffyRoom, DarkStuffyRoom.getDescription()+" Половину занимала "+bed.name+" под "+quiltWhiteBlanket.name+", ");
        vincent.setMemory(burner, burner.name+", ");
        vincent.setMemory(shelf, shelf.name+".");
        vincent.setMemory(entryway, "На "+entryway.getDescription()+" была ");
        vincent.setMemory(brownCeramicSink, brownCeramicSink.name+", одна на несколько семей.");
